...
(define (sortname stringlist)
  (string-append (firstname stringlist) " " (lastname stringlist)))
...
(define (compute-avgs lst)
  (define (quiz-avg lst)
    (exact->inexact (/ (apply + lst) 100)))
  
  (define (exam-avg lst)
    (exact->inexact (/ (apply + lst) 300)))
  ...
  (if (empty? lst)
      empty
      (let
          ([first-name (first lst)]
          [last-name (first (rest lst))]
          [grade (/ (floor(* (weighted-avg (map string->number (rest (rest lst)))) 10000)) 100)])
          (list last-name first-name grade (letter-grade grade)))))
